...
function checkValidation(req, res, next) {
  ...
  var errorsList = [];
  if (
    validator.isFloat(req.body.actual_payment, {
      min: -99999999,
      max: 99999999,
    }) === false
  ) {
    errorsList.push({
      ...
      message: "Validation number or range failed",
    });
  } else {
  ...
  if (invoiceItemList && invoiceItemList.length) {
    for (var i = 0; i < invoiceItemList.length; i++) {
      ...
      if (
        validator.isInt(item.quantity) === false ||
        item.quantity < 0 ||
        item.quantity > 99999999
      ) {
        errorsList.push({
          path: ["invoiceItemList", i, "quantity"],
          type: "Int Validation",
          message: "Validation number or range failed",
        });
      }
      if (
        validator.isFloat(item.price_net, { min: 0, max: 99999999 }) === false
      ) {
        errorsList.push({
          path: ["invoiceItemList", i, "price_net"],
          type: "Number Validation",
          message: "Validation number or range failed",
        });
      } else {
      ...
      if (
        validator.isInt(item.vat_rate) === false ||
        item.vat_rate < 0 ||
        item.vat_rate > 100
      ) {
        errorsList.push({
          path: ["invoiceItemList", i, "vat_rate"],
          type: "Int Validation",
          message: "Validation number or range failed",
        });
      }
...
router.get("/invoice/:id", function (req, res, next) {
  req.app
    .get("components")
    .invoice.detail({ id: req.params.id }, function (err, data) {
      if (err) {
        next(err);
      } else {
...
router.get("/invoice/:id/pdf", function (req, res, next) {
  req.app
    .get("components")
    .invoice.generate(
      { id: req.params.id, account: req.app.get("config").account },
      function (err, stream) {
        if (err) {
          next(err);
        } else {
...
router.post("/invoice", checkValidation, function (req, res, next) {
  new Promise(function (resolve) {
    ...
    .then(function (invoiceId) {
      req.app
        .get("components")
        .invoice.detail({ id: invoiceId }, function (err, data) {
          if (err) {
            next(err);
          } else {
            if (data.success === true) {
